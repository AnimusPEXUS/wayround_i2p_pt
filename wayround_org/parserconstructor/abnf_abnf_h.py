...
def parse_next_rulelist(text, start, error_log):
    ...
    if ret is not None:
        ...
        if res is None:
            ...
            if len(res) < 1:
                error_log.append(
                    wayround_org.parserconstructor.utils.ErrorNote(
                        "c-nl count must be one or more",
                        start
                        )
                    )
                ret = None
...
def parse_next_defined_as(text, start, error_log):
    ...
    start = parse_next_c_wsp_any_number(text, start, error_log, ret)
...
def parse_next_comment(text, start, error_log):
    ...
    if ret is not None:
        ...
        res = parse_next_CRLF(text, start, error_log)
...
def parse_next_concatenation(text, start, error_log):
    ...
    if ret is not None:
        ...
        while True:
            ...
            _start = parse_next_c_wsp_any_number(text, start, error_log, ret)
...
def parse_next_x_val(text, start, error_log, prefix, target_node):
    ...
    if ret:
        _start = parse_next_x_any_number(
            text, start, error_log, digit_parser, target_node)
    ...
    if ret:
        if text[start] == '.' and digit_parser(
                text, start + 1, []) is not None:
