...
import wayround_i2p.pt.abnf_core_h


RULES = wayround_i2p.pt.utils.Rules(
    {
    ...
    [wayround_i2p.pt.abnf_core_h.RULES]
    )
...
def parse_next_rulelist(text, start, error_log):
    ...
    while True:
        ...
        res = wayround_i2p.pt.utils.any_number(
            text,
            start,
            error_log,
            wayround_i2p.pt.utils.or_,
            [
                parse_next_c_wsp
                ]
            )
...
def parse_next_rule(text, start, error_log):
    ...
    res = wayround_i2p.pt.utils.and_(
        text,
        start,
        error_log,
        [
...
def parse_next_hyphen(text, start, error_log):
    ret = wayround_i2p.pt.utils.parse_next_re(
        text,
        start,
        error_log,
        r'\-'
...
def parse_next_defined_as(text, start, error_log):
    ...
    res = wayround_i2p.pt.utils.parse_next_re(
...
def parse_next_group(text, start, error_log):
    ...
    if res is None:
        error_log.append(
            wayround_i2p.pt.utils.ErrorNote(
